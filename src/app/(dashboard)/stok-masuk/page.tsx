...
export default function StokMasukPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isNaN(qtyNum) || qtyNum < 1) {
      toast.error("Jumlah stok masuk minimal 1");
      return;
    }

    startTransition(async () => {
      try {
        await addStockIn(parseInt(selectedItemId), qtyNum, note || undefined);
        toast.success("Stok masuk berhasil dicatat!");
        setQuantity("1");
        setNote("");
        loadData();
      } catch (error: any) {
        toast.error(error.message || "Gagal mencatat stok masuk");
      }
    });
  };

  const selectedItem = items.find((i) => String(i.id) === selectedItemId);
  const totalQuantityIn = movements.reduce((acc, curr) => acc + curr.quantity, 0);
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 no-print">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Catat Stok Masuk</h1>
          <p className="text-slate-500 mt-1">
            Pencatatan barang produksi yang baru masuk ke gudang
          </p>
        </div>
        <Button
          variant="outline"
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                <Label htmlFor="item-select" className="text-slate-700 font-medium">
                  Pilih Barang
                </Label>
                <Select
                ...
                >
                  <SelectTrigger id="item-select" className="mt-1 w-full">
              ...
              <div>
                <Label htmlFor="qty-input" className="text-slate-700 font-medium">
                  Jumlah Masuk ({selectedItem?.unit || "Unit"})
                </Label>
                <Input
                  id="qty-input"
                  type="number"
              ...
              <div>
                <Label htmlFor="note-input" className="text-slate-700 font-medium">
                  Catatan / Keterangan (Opsional)
                </Label>
                <Input
                  id="note-input"
                  placeholder="Contoh: Dari Supplier PT XYZ / Po #123..."
                  value={note}
            ...
            <div className="flex justify-end pt-2">
              <Button
                ...
                className="bg-gradient-to-r from-emerald-600 via-teal-600 to-emerald-700 hover:from-emerald-700 hover:to-teal-800 text-white font-semibold shadow-md shadow-emerald-500/20 px-6"
              >
                {isPending ? (
                  <>
      ...
      <div className="flex items-center justify-between pt-2 no-print">
        ...
        <div className="flex items-center gap-2 text-xs font-medium self-end sm:self-auto">
          ...
          <span className="bg-emerald-50 text-emerald-800 px-3 py-1.5 rounded-full border border-emerald-200 font-semibold">
            Total Masuk: +{totalQuantityIn} Unit
          </span>
        </div>
      </div>
        ...
        <CardContent className="p-0">
          {loading ? (
            ...
            <div className="text-center py-16">
              <ArrowDownToLine className="w-16 h-16 text-muted-foreground mx-auto mb-4 opacity-40" />
              <h3 className="text-lg font-medium text-slate-800">
                Belum ada catatan stok masuk
              </h3>
              <p className="text-sm text-slate-500 mt-1">
                Gunakan form di atas untuk mencatat penerimaan barang baru.
              </p>
            </div>
          ) : (
              ...
              <Table>
                <TableHeader>
                  <TableRow className="bg-slate-50/80">
                    ...
                    <TableHead className="text-right">Jumlah Masuk</TableHead>
                    ...
                    return (
                      <TableRow key={mov.id} className="hover:bg-slate-50/80 transition-colors">
                        ...
                        <TableCell className="text-right">
                          <Badge className="bg-emerald-100 text-emerald-800 border border-emerald-200 hover:bg-emerald-100 text-xs font-bold px-2.5 py-0.5">
      ...
      {/* Formal Paper Document Print Layout for Stock In */}
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
            <div>
              ...
              <p style={{ fontSize: "8.5pt", color: "#555", margin: "2px 0 0 0" }}>
                Divisi Pergudangan & Manajemen Persediaan Barang Produksi
              </p>
            </div>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "9pt", marginBottom: "25px", background: "#ffffff" }}>
          <thead>
            <tr style={{ background: "#ffffff" }}>
              ...
              <th style={{ border: "1.5px solid #000", padding: "6px", width: "100px", textAlign: "right", background: "#ffffff" }}>JUMLAH MASUK</th>
              ...
              <th style={{ border: "1.5px solid #000", padding: "6px", textAlign: "left", background: "#ffffff" }}>CATATAN / SUPPLIER</th>
            </tr>
          </thead>
          <tbody>
          ...
          <tfoot>
            <tr style={{ background: "#ffffff", fontWeight: "bold" }}>
              <td colSpan={2} style={{ border: "1.5px solid #000", padding: "6px 8px", textAlign: "right", background: "#ffffff" }}>TOTAL PENERIMAAN:</td>
              <td style={{ border: "1.5px solid #000", padding: "6px 8px", textAlign: "right", background: "#ffffff" }}>+{totalQuantityIn}</td>
              <td colSpan={4} style={{ border: "1.5px solid #000", padding: "6px 8px", background: "#ffffff" }}>({movements.length} Transaksi Penerimaan)</td>
            </tr>
          </tfoot>
        </table>
            ...
            <tbody>
              <tr>
                <td style={{ border: "none", width: "33.33%", textAlign: "center", verticalAlign: "top", padding: "0 10px", background: "#ffffff" }}>
                  <p style={{ fontSize: "9pt", fontWeight: "bold", margin: "0 0 60px 0" }}>Petugas Penerima Gudang</p>
